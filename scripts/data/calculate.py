...
if __name__=='__main__':
    ...
    tp  = calculator.regrid(tp).where(lambda x:x>=1e-4,0.0).load()
    pr  = calculator.regrid(pr).where(lambda x:x>=1e-4,0.0).load()
    ...
    tp  = calculator.resample(tp,'sum')
    pr  = calculator.resample(pr,'mean')
