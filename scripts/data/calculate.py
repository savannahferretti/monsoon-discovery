...
if __name__=='__main__':
    ...
    logger.info('Resampling/regridding variables to 3-hourly...')
    ps  = calculator.regrid(ps).load()
    t   = calculator.regrid(t).load()
    q   = calculator.regrid(q).load()
    lf  = calculator.regrid(lf).load()
    lhf = calculator.regrid(lhf).load()
    shf = calculator.regrid(shf).load()
    tp  = calculator.regrid(tp).clip(min=0).load()
    pr  = calculator.regrid(pr).clip(min=0).load()
    ps  = calculator.coarsen_3h(ps,'first')
    t   = calculator.coarsen_3h(t,'first')
    q   = calculator.coarsen_3h(q,'first')
    lhf = calculator.coarsen_3h(lhf,'mean')
    shf = calculator.coarsen_3h(shf,'mean')
    tp  = calculator.coarsen_3h(tp,'sum')
    pr  = calculator.coarsen_3h(pr,'mean')
