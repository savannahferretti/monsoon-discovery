...
class BaselineNN(torch.nn.Module):
    ...
    def __init__(self,nfieldvars,nlevs,nlocalvars,hasmask=False):
        '''
        Purpose: Initialize a baseline neural network that flattens vertical profiles and concatenates local inputs.
        Args:
        ...
        - nlocalvars (int): number of local input variables (e.g. lf, shf, lhf)
    ...
    def forward(self,fields,local,mask=None):
        ...
        if self.hasmask and mask is not None:
            ...
            X = torch.cat([X.flatten(1),local],dim=1)
        else:
...
class KernelNN(torch.nn.Module):
    ...
    def forward(self,fields,dlev,local,mask=None):
        '''
